...
bool disarmFailpointThrow(const char *name)
{
    const char *known = findIn(KnownThrowFailpoints, name);
    if (known == nullptr)
        return false;
    // Only this point: another one armed in between stays armed.
    return armedThrow.compare_exchange_strong(known, nullptr,
